...
def screen_overview() -> None:
    ...
    risk_index = st.session_state.signals["composite_risk_score"].mean()
            ...
            if alerts.empty:
                ...
                for _, r in alerts.head(12).iterrows():
                    ...
                    if st.button("Open details", key=btn_key, use_container_width=True):
                        st.session_state.selected_commodity = r["commodity"]
    ...
    with st.container(border=True):
        ...
        show_cols = [
            "commodity", "market", "date", "price", "chg_7d", "chg_30d",
            "supply_risk_score", "logistics_risk_score", "climate_risk_score", "geopolitical_risk_score",
            "composite_risk_score", "main_driver", "confidence",
        ]
...
def screen_drilldown() -> None:
    ...
    default_idx = commodity_list.index(st.session_state.selected_commodity) if st.session_state.selected_commodity in commodity_list else 0
...
"""
    ...
    with st.container(border=True):
        ...
        fig.add_trace(go.Scatter(x=t["date"], y=t["composite_risk_score"], name="Composite Risk", mode="lines+markers", yaxis="y2"))
        fig.update_layout(
            height=340,
    ...
    with st.container(border=True):
        scen = pd.DataFrame(
            [
                {"Scenario": "Best", "Probability": 0.20, "Assumption": "Freight normalizes; no new export restrictions", "Projected Risk": max(0, comp - 10)},
                {"Scenario": "Base", "Probability": 0.55, "Assumption": "Status quo volatility persists", "Projected Risk": comp + 2},
                {"Scenario": "Worst", "Probability": 0.25, "Assumption": "Chokepoint disruption + policy shocks", "Projected Risk": min(100, comp + 14)},
            ]
        )
    ...
    if st.button("Simulate Red Sea Disruption (Qatar)", use_container_width=True):
        ...
        shock_df.loc[mask, "geopolitical_risk_score"] = (shock_df.loc[mask, "geopolitical_risk_score"] + 15).clip(0, 100)
...
def screen_operations() -> None:
    ...
    with st.container(border=True):
        ...
        with c3:
            commodity = st.selectbox("Commodity", ["All"] + sorted(st.session_state.signals["commodity"].unique().tolist()))
                ...
                with st.container(border=True):
                    ...
                    with top[1]:
                        new_status = st.selectbox(
                            ...
                            index=["Open", "In Progress", "Blocked", "Done"].index(r["status"]) if r["status"] in ["Open","In Progress","Blocked","Done"] else 0,
                            key=f"status_{r['id']}",
                        )
                    with top[2]:
...
screen = st.sidebar.radio(
    ...
    index=["Qatar Food Security Overview", "AI Issue Breakdown", "Action & Intervention Tracking"].index(st.session_state.screen)
    if st.session_state.screen in ["Qatar Food Security Overview", "AI Issue Breakdown", "Action & Intervention Tracking"]
    else 0,
)
